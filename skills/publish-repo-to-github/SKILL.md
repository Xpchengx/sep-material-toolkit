...
### ⚠️ 第四次了：注释、示例、配置模板都是高发区

前三处是「表格里的示例」（文档、对照表）。第四次命中 **7 个文件**，
全在**不该有真名的地方**：

| 位置 | 写进去的东西 |
|---|---|
| `README.md` | 举例的文件名「上海XX材料库存(10月26日).xlsx」 |
| `index.html` | 占位提示里的 `XX_库存_20260908.xlsx` |
| 源码注释 | `# {'A': '某集成商', ...}` 这种"说明性"注释 |
| 配置模板 `.example.json` | 真实台账路径 + 真实集成商名 |
| 前端 JS 注释 | 「避免「某名」误配到「某名通讯」」 |

**规律：越是不参与逻辑、只是"说明"的地方越容易写真名。**
这些地方**恰恰最容易漏扫** —— 因为它们看起来"只是注释"。

**做法**：
1. 写示例/注释/模板时，**第一反应就用 `XX`、`集成商A`、`D:\台账\`** 这类占位符
2. 提交前的扫描是**最后一道**，不是唯一一道
3. 扫描要按**字节**扫（`open(p,'rb')`），不能用文本模式 —— 二进制资源会让
   `subprocess(text=True)` 抛 `UnicodeDecodeError`，导致**整批文件被静默跳过**
...
**工作目录里往往同时在放别的任务的文件。** 黑名单列不胜列，每次新增都要补一行 ——
漏一次就可能把不相干内容推到公开仓库。

真实事故：一次提交前断言拦下 **46 个文件**，多出的 13 个是用户其它任务的脚本与导出文本
（`build_annex_v3.py`、`patch_docx_v3.py`、`jf_ppt2.txt` …）。

改成**默认拒绝**：

```gitignore
# 根目录只放行本项目自己的文件，其余一律忽略
/*
!/.gitignore
!/README.md
!/index.html
...
要点：
- `/*` 忽略根目录所有条目，`!` 逐条放行
- 放行**目录**时用 `!/dir/`，之后该目录内部仍按普通规则匹配
- **新增本项目文件时必须补一条 `!`** —— 否则它会静默地不进版本库（比误上传更难发现）

> 判据：**问一句「这个文件的内容是谁的数据」。** 生成物 ≠ 无害；
> 生成物里装的是业务数据（型号归一表、安全库存），就必须 ignore。
